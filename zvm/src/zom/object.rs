...
#[derive(
    Debug, Clone, Copy, Default, Archive, Serialize, Deserialize, PartialEq, Eq, PartialOrd, Ord,
)]
#[rkyv(derive(Debug, Clone, Copy, Default), compare(PartialEq, PartialOrd))]
pub struct ModuleVersion([u8; 4]);

impl ModuleVersion {
    pub const fn new(release: u8, major: u8, minor: u8, patch: u8) -> Self {
        Self([release, major, minor, patch])
    }

    pub const fn release() -> u8 {
        self.0[0]
    }

    pub const fn major() -> u8 {
        self.0[1]
    }

    pub const fn minor() -> u8 {
        self.0[2]
    }

    pub const fn patch() -> u8 {
        self.0[3]
    }
}

#[derive(Debug, Clone, Archive, Serialize, Deserialize)]
#[rkyv(derive(Debug, Clone, Default), compare(PartialEq, PartialOrd))]
pub enum ConstVal {
    Bool(bool),
    Char(char),
    Byte(u8),
    ...
    Rune(PoolStringAddr),
    Symbol(PoolStringAddr),
    PtrSize(isize),
    UptrSize(usize),
}

/// single whitespace delimited String pool
/// all strings are unique when built with StringPoolBuilder
#[derive(Debug, Clone, Archive, Serialize, Deserialize)]
...
// #[repr(C)] pub struct ByteChunk { pub begin: u32,
//     pub size_bytes: u32,
// }
//
// impl ByteChunk {
