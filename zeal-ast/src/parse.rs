...
impl Parser {
    ...
    fn place_value_block(&mut self, ty: BlockType) -> anyhow::Result<ExprStmt> {
        let bl = match ty {
            BlockType::WhenBranch => {
                try_block!(self, TokType::FatArrow, TokType::Comma)
            }
            BlockType::WhenElse => {
                let otherwise = try_block!(self, TokType::FatArrow, TokType::End | TokType::Comma);
    ...
    fn call_args(&mut self, call_head: Expr) -> anyhow::Result<Expr> {
        ...
        if let TokType::CloseParen = self.peek_ty() {
            let ec = Expr::call(call_head, args);
            return Ok(ec);
        }
        ...
        while let TokType::Comma = self.peek_ty() {
            ...
            matches!(self.peek_ty(), TokType::CloseParen),
    ...
    fn primary(&mut self) -> anyhow::Result<Expr> {
        match self.peek().tok {
            ...
            LexTok::Ident => {
                let name = self.peek().clone().lexeme;
                self.adv(1)?;
